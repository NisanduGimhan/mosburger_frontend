...
function renderMenu(filteredItems) {
  const menuContent = document.getElementById("menue-content");
  menuContent.innerHTML = ""; // Clear existing content.
  menuContent.classList.add("row", "row-cols-1", "row-cols-md-3", "g-4");

  filteredItems.forEach((filterItem) => {
    const item = items.find((item) => item.itemno === filterItem.itemno);
    if (item) {
      const card = document.createElement("div");
      card.classList.add("col");
      card.innerHTML = `
        ...
        item.name
                ...
                <div class="card-body">
                    <h5 class="card-title">${item.name}</h5>
                    <p class="card-text">Rs.${item.price}</p>
                    <button style="background-color: #FB710C;" class="btn btn-add-item" onclick="addToCart(${items.indexOf(
                      item
                    )})">Add to Cart</button>
                </div>
            </div>
...
//     filteredItems.forEach((filterItem) => {
//       // Filter the items array to match the current filterItem by itemno
//       const item = items.find((item) => item.itemno === filterItem.itemno);
...
//                   <h5 class="card-title">${item.name}</h5>
//                   <p class="card-text">Rs.${item.price}</p>
//                   <button style="background-color: #FB710C;" class="btn btn-add-item" onclick="addToCart(${items.indexOf(item)})">Add to Cart</button>
//               </div>
//           </div>
//         `;
//         menuContent.appendChild(card);
...
function filterCategory(category) {
  ...
  if (category === "All") {
    renderMenu(items); // Render all items
  } else {
    const filteredItems = items.filter((item) => item.itemtype === category);
    renderMenu(filteredItems); // Render filtered items
  }
}
...
//             <span>Rs.${(parseFloat(cartItem.price) * cartItem.quantity).toFixed(
//               2
//             )}</span>
...
function placeOrder() {
  ...
  let existingCustomer = customers.find(
    (customer) => customer.name === customerName && customer.phone === contactNo
  );
